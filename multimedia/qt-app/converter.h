...
static int convert_to_rgb32(char * arr, int ratio)
{
    int i, j;
    int wp = 0;
    unsigned int rgb888;
    ...
    for(i=0; i<480; i+=inc)
    {
        for(j=0; j<320; j+=inc)
        {
            ...
            //*(unsigned int *)(arr+1024*4*i/inc+y*1024*4+(j*2/inc)*4+x*4) = rgb888;
            rgb32[wp++] = rgb888;
            Y = (yuv422>>16)&0xff;
            ...
            //*(unsigned int *)(arr+1024*4*i/inc+y*1024*4+(j*2/inc+1)*4+x*4) = rgb888;
            rgb32[wp++] = rgb888;
        }
    }
