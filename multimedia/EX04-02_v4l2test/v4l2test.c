...
static int draw_rect(int x, int y, int w, int h, unsigned long color, struct fb_var_screeninfo *vip, struct fb_fix_screeninfo *fip, char *map)
{
	int xx, yy;
	long location = 0;
...
static int capture_to_fb(const void *p, int size, int x, int y, int ratio)
{
	...
	for(i=0; i<480; i+=inc)
	{
		for(j=0; j<320; j+=inc)
		{
			yuv422 = *(unsigned int *)(pp+i*640*2+j*4);
...
// 캡처가 완료된 이미지를 LCD에 뿌린다.
// p라는 변수에 YCbCr형식의 색정보가 들어가 있다.
...
static void start_capturing(void)
{
	...
	switch (io) {
		...
		case IO_METHOD_USERPTR:
		// 프레임버퍼를 4개나 만들어서 사용하는 이유는?
		// 단일 버퍼를 사용할시에는, LCD 컨트롤러에 읽고 쓰는 동작이 겹친다.
		// flickering현상이 일어난다.(깜빡거림.)
		// 혹은, 영상 데이터를 LCD에서 읽어오고 있는 도중에, 웹캠이 이미지를 저장해서
		// 움직임이 심한 영상일 경우, 이상하게 나오게 된다.
		for (i = 0; i < n_buffers; ++i) {
			struct v4l2_buffer buf;
...
static void init_mmap(void)
{
	...
	// 데이터를 저장할 메모리 공간을 미리 만들어 놓는다.
	// 이렇게 함으로써, LCD에 빨리 데이터를 쓸 수 있다.
	req.count = 4;
